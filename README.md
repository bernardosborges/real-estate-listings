...
This project includes a full observability stack for metrics and logs. It provides insight into business event, performance, reliability, and system health.

### Metrics
- **Prometheus**: Scraps metrics from the backend, containers, and host (`/metric` via FastAPI Instrumentator, cAdvisor, Node Exporter).
- **Grafana**: Visualizes metrics and alerts in dashboards. Default dashboards include backend APM metrics, containers and host infrastructure metrics, business metrics, and active alerts.

### Logs
- **Loki**: Centralized log aggregation for Docker services (`backend`, `nginx`, `db`).
- **Promtail**: Collects stdout/strerr from Docker containers and forwards them to Loki. Applies relabeling for service, container, image, and log level.

**Usage (all observability services via Docker Compose):**
```bash
docker-compose --env-file .env.local -f docker-compose.dev.yml up -d prometheus grafana loki promtail
```
...
- Loki ([docs](https://grafana.com/docs/loki/latest/)) - Centralized log storage
- Promtail ([docs](https://grafana.com/docs/loki/latest/clients/promtail/)) - Log collector
- Prometheus FastAPI Instrumentator ([GitHub](https://github.com/trallnag/prometheus-fastapi-instrumentator)) - Application metrics
