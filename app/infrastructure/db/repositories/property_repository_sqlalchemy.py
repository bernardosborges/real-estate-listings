...
class PropertyRepositorySQLAlchemy(PropertyRepository):
    ...
    def exists_by_public_id(self, public_id: PropertyPublicId) -> bool:
        stmt = select(exists().where(PropertyModel.public_id == str(public_id)))
        return self.db.execute(stmt).scalar()

    def deactivate(self, id: int) -> None:
        stmt = (
            update(PropertyModel)
            .where(PropertyModel.id == id, PropertyModel.deleted_at.is_(None))
            .values(updated_at=datetime.now(timezone.utc), is_active=False)
        )
    ...
    def activate(self, id: int) -> None:
        stmt = (
            update(PropertyModel)
            .where(PropertyModel.id == id, PropertyModel.deleted_at.is_(None))
            .values(updated_at=datetime.now(timezone.utc), is_active=True)
        )
        self.db.execute(stmt)
