...
class PropertyTagRepository:
    ...
    @staticmethod
    def get_by_property_and_tag(
        db: Session, property_id: int, tag_id: int, include_deleted: bool = False
    ) -> PropertyTagModel | None:
        query = db.query(PropertyTagModel).filter(
            PropertyTagModel.property_id == property_id,
            PropertyTagModel.tag_id == tag_id,
        )
    ...
    def restore(db: Session, property_id: int, tag_id: int) -> PropertyTagModel | None:
        db_property_tag = PropertyTagRepository.get_by_property_and_tag(
            db, property_id=property_id, tag_id=tag_id, include_deleted=True
        )
        if not db_property_tag:
            return None
    ...
    def hard_delete_exclusive_group(
        ...
        query = (
            ...
            .filter(
                PropertyTagModel.property_id == property_id,
                PropertyTagModel.group_id == group_id,
            )
