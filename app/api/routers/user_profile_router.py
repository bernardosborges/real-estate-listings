...
def create_user_profile_endpoint(
    ...
    return UserProfileService.create(
        db, current_user.id, **payload.model_dump(exclude_unset=True), current_user=current_user
    )
...
def get_user_profile_endpoint(
    public_id: str, db: Session = Depends(get_db_session), current_user: UserModel = Depends(get_authenticated_user)
):
