...
from app.schemas.photo_schema import (
    PhotoReadSchema,
    PhotoUpdateSchema,
    PhotoPresignedUrlUploadRequestSchema,
    PhotoPresignedUrlUploadResponseSchema,
    PhotoMarkUploadedResponseSchema,
)
...
def create_photo_presigned_upload_url_endpoint(
    ...
    return PhotoService.generate_presigned_upload_url(
        db, storage, property_public_id, **payload.model_dump(exclude_unset=True), current_user=current_user
    )
...
@router.get(
    "/{photo_id}", response_model=PhotoReadSchema, summary="Get a photo", description="Get a photo from the database."
)
def get_photo_by_id_endpoint(
    photo_id: int, db: Session = Depends(get_db_session), current_user: UserModel = Depends(get_authenticated_user)
):
...
def list_photo_by_property_endpoint(
    property_id: int, db: Session = Depends(get_db_session), current_user: UserModel = Depends(get_authenticated_user)
):
