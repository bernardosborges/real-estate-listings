...
def list_tag_group_endpoint(
    db: Session = Depends(get_db_session), current_user: UserModel = Depends(get_authenticated_user)
):
...
def restore_tag_group_endpoint(
    tag_group_slug: str, db: Session = Depends(get_db_session), current_user: UserModel = Depends(get_superuser)
):
...
def delete_tag_group_endpoint(
    tag_group_id: int, db: Session = Depends(get_db_session), current_user: UserModel = Depends(get_superuser)
):
