...
def register_user_endpoint(
    ...
    register_usecase = RegisterUserUseCase(
        user_repository=user_repository, profile_repository=profile_repository, password_hasher=password_hasher
    )
...
def login_user_endpoint(user_input: LoginRequestSchema, db: Session = Depends(get_db_session)):
    user_repository: UserRepository = UserRepositorySQLAlchemy(db)
    token_service: TokenService = JWTTokenService()
    password_hasher: PasswordHasher = BcryptPasswordHasher()

    login_usecase = LoginUserUseCase(
        user_repository=user_repository, token_service=token_service, password_hasher=password_hasher
    )
