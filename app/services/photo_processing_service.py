...
# -----------------------------------------------
    ...
    def process(self, photo_public_id: str) -> None:
        db_photo =  PhotoService._get_by_public_id_or_404(self.db, photo_public_id, is_active=None, include_deleted=False)

        # Idempotency guard
        if db_photo.processing_status != PhotoProcessingStatusEnum.PENDING:
            logger.info("Skipping photo processing - invalid state", extra={"photo": photo_public_id, "status": db_photo.processing_status})
        ...
        try:
            ...
            width, height = ImageValidator.validate_and_extract_image_dimensions(image, limits["max_width"], limits["max_height"])
            ...
            self.db.rollback()
            logger.exception("Unexpected error during photo processing.", extra={"photo": photo_public_id})
    ...
    def _mark_ready(self, photo: PhotoModel, optimized_key: str, thumb_key: str, width: int, height: int) -> None:
